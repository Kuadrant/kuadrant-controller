...
func (r *RateLimitPolicyReconciler) reconcileWithVirtualService(ctx context.Context, vs *istio.VirtualService, rlp *apimv1alpha1.RateLimitPolicy) error {
	...
	for _, gw := range vs.Spec.Gateways {
		...
		if err := r.Client().Get(ctx, ratelimitsPatchKey, ratelimitsEnvoyFilter); err != nil {
			...
			ratelimitsEnvoyFilter, err = desiredRatelimitsEnvoyFilter(rlp, vs.Spec.Hosts, gwKey, gwLabels)
			if err != nil {
...
func desiredRatelimitsEnvoyFilter(rlp *apimv1alpha1.RateLimitPolicy, vHostNames []string, gwKey client.ObjectKey, gwLabels map[string]string) (*istio.EnvoyFilter, error) {
	var patches []*networkingv1alpha3.EnvoyFilter_EnvoyConfigObjectPatch
